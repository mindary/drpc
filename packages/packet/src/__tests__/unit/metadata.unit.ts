...
import {Metadata, MetadataObject, MetadataValue} from '../../metadata';
...
class TestMetadata extends Metadata {
  ...
  static fromHttp2Headers(headers: http2.IncomingHttpHeaders): TestMetadata {
    const result = Metadata.from(headers) as TestMetadata;
...
describe('Metadata', () => {
  ...
  describe('fromHttp2Headers', () => {
    it('creates a Metadata object with expected values', () => {
      ...
      const data = metadataFromHeaders.getInternalRepresentation();
    ...
    it('creates an empty Metadata object from empty headers', () => {
      ...
      const data = metadataFromHeaders.getInternalRepresentation();
      assert.deepStrictEqual(data, new Map<string, MetadataValue[]>());
    });
  });
});
